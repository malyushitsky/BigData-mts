...
### 33) Откроем в браузере интерфейс hadoop и проверим что все ноды живы с помощью проброски портов
##### 33.1) Подключаемся к серверу с учетом проброшеннего порта
```bash
ssh -L 9870:team-19-nn:9870 team@176.109.91.21
```
##### 33.1) Проверка hadoop интерфейса (в браузере ввести)
```bash
127.0.0.1:9870
```
