...
### 1. Разархивация архива с Hadoop (внутри пользователя хадуп в корень файловой системы, архив дистрибутива скачивался ранее в task-1)
```bash
tar -xvzf hadoop-3.4.0.tar.gz
```

### 2. Добавление переменных окружения
#### 2.1 Открытие нужного файла
```bash
nano ~/.profile
```
#### 2.2 Добавление в конец файла следующих строки для настройки окружения Hadoop
```plaintext
...
### 3. После редактирования файла применяем изменения
```bash
source ~/.profile
```

### 4. Проверка установки Hadoop
```bash
hadoop version
```

### 5. Переход в папку дистрибутива хадуп
```bash
